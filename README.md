...
使用方法：在你的模组的scripts文件夹下放置`ATD.js`文件，然后在你的星球JS脚本中添加: 
``` js
...
以下是几个添加示例：
  ...
  把赛普罗星球的所有内容添加到myplanet星球: 
``` js
ATD.AddPlanetToDatabase(Planets.serpulo, myplanet);
``` 
  把模组名为"模组"的MOD里所有内容添加到myplanet星球：
``` js
ATD.AddAllToDatabase("模组", myplanet);
```
...
Here are some addition examples:
  ...
  Add all content from the Serpulo planet to myplanet: 
``` js
ATD.AddPlanetToDatabase(Planets.serpulo, myplanet);
```
  Add all content from a mod named "模组" to myplanet:
``` js
ATD.AddAllToDatabase("模组", myplanet);
```
